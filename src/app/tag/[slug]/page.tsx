...
export default function TagPage() {
  ...
  return (
    <div className="xl:col-span-8 space-y-8">
      {/* Header */}
      <div className="relative overflow-hidden p-8 rounded-[2rem] bg-gradient-to-br from-emerald-600 to-green-700 text-white shadow-xl shadow-emerald-500/20">
         <div className="relative z-10 space-y-4">
            <Link href="/tags" className="inline-flex items-center gap-2 text-emerald-100 hover:text-white font-bold text-sm transition-colors group">
            ...
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <p className="text-emerald-100 font-medium">Đang có {tag._count.threads} bài viết liên quan</p>
               </div>
            </div>
         </div>
          ...
          <div className="space-y-4">
            {tag.threads.map((thread) => (
              <motion.div 
                ...
                className="group cursor-pointer p-6 rounded-2xl bg-white dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 hover:border-emerald-500/30 hover:shadow-lg hover:shadow-emerald-500/5 transition-all"
              >
                <div className="flex gap-5">
                   ...
                   <div className="flex-1 space-y-3">
                      <div className="space-y-1">
                         <h3 className="text-lg font-bold text-zinc-100 group-hover:text-emerald-600 transition-colors leading-tight">
                            {thread.title}
                         </h3>
                         <div className="flex items-center gap-3 text-xs text-zinc-500 font-bold">
                            ...
                            <Link href={`/user/${thread.author.username}`} onClick={(e) => e.stopPropagation()} className="flex items-center gap-1 hover:text-emerald-600 transition-colors">
                   ...
                   <div className="shrink-0 flex flex-col justify-center items-end gap-1">
                      ...
                      <div className="text-xl font-black text-zinc-100">{thread._count.posts}</div>
                   </div>
                </div>
              </motion.div>
            ))}
